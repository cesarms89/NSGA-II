#Tercera tecnica de cruza, ofrece mejores resultados
SBX <- function(p1, p2, limInf, limSup, numVar) {
  ...
  for (i in 1:numVar) {
    ...
    #Inicio condicion 1ª variable ZDT4
    # if (i==1) {
    #   limInf <- 0
    #   limSup <- 1
    # }
    ...
    #   limSup <- limSupBk
    # }
    #Fin condicion 1ª variable ZDT4
    
    #probabilidad de muta del 1%
    if (runif(1) <= 0.01) {
      ...
      B1 <- 1 + ((2*(x2 - limInf)) / x1 - x2)
      B2 <- 1 + ((2*(limSup - x1)) / x1 - x2)
...
#Primera tecnica de cruza, fue empleada en Schaffer
CruzaSimple <- function(p1, p2, limInf, limSup) {
  ...
  c2 <- MutaPolinomial(p2, limInf, limSup)
  return(c(c1, c2))
}

#Segunda tecnica de cruza, mejor que la primera pero peor que sbx
...
#Complemento Evolucion Diferencial
